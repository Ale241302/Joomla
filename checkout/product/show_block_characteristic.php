	...
	?>
		<script>
			...
			function initVariants() {
				var allRows = document.querySelectorAll('tr[data-characrow]'),
					first = true,
					qtyArea = document.getElementById('hikashop_product_quantity_main'),
					altArea = document.getElementById('hikashop_product_quantity_alt');
				...
				<?php if (@$this->displayVariants['prices']) { ?>
					...
					priceDivs.forEach(function(sub) {
					...
					if (firstEl.tagName.toLowerCase() == 'select') {
						if (firstEl.options.length == 2) {
							...
							if (window.jQuery && typeof(jQuery().chosen) == "function") {
								jQuery("#hikashop_product_characteristics select").chosen('destroy').chosen({
			...
			function getValidVariants(pos) {
				var allInputs = document.querySelectorAll('[data-characteristic]'),
					selectedElements = [],
					validVariants = [];
			...
			function hikashopVariantSelected(obj) {
				...
				?>
					...
					if (!obj)
						return true;
					var pos = obj.getAttribute('data-characteristic'),
						last = obj.getAttribute('data-last'),
						otherRow = null,
						qtyArea = document.getElementById('hikashop_product_quantity_main'),
						altArea = document.getElementById('hikashop_product_quantity_alt');
					if (!last) {
			...
			function hikashopUpdateVariant(obj) {
				...
				if (window.Oby && window.Oby.fireAjax) window.Oby.fireAjax("hkAfterUpdateVariant", {
					obj: obj,
					selection: selection
				});
				return true;
					...
					?>
						...
						<script>
							document.addEventListener('DOMContentLoaded', function() {
								var inputField = document.getElementById('hikashop_listing_quantity_<?php echo $this->row->product_id; ?>');
								var characteristicValue = '<?php echo $variantCharacteristic->characteristic_value; ?>';
								var braValue = characteristicValue.split('|')[0].trim(); // Obtener el valor de BRA

								inputField.addEventListener('change', function() {
									var value = parseInt(this.value, 10);
									var braNumber = parseInt(braValue.split(' ')[1], 10);
									...
									if (braNumber >= 33 && braNumber <= 36 || braNumber >= 45 && braNumber <= 47) {
										if (value % 5 !== 0) {
						...
						<td class="hikashop_product_add_to_cart_row hikashop_variants_table_td">
							...
							?>
								...
								?>
									...
									?>
											<input id="hikashop_listing_quantity_<?php echo $this->row->product_id; ?>" type="text" style="width:60px; text-align: center;" name="data[<?php echo $this->row->product_id; ?>]" class="hikashop_listing_quantity_field" value="0" />
